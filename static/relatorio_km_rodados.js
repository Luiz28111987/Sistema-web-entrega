document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('relatorioKmRodados').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch('/relatorio_km_rodados', {
                method: 'POST',
                headers: {
            ...
            resultados.forEach(resultado => {
                ...
                row.innerHTML = `
                    <td>${resultado[0]}</td> <!-- Data da Entrega -->
                    <td>${resultado[1]}</td> <!-- Motorista -->
                    <td>${resultado[2]}</td> <!-- Placa -->
                    <td>${resultado[3]}</td> <!-- Veículo -->
                    <td>${resultado[4]}</td> <!-- KM Rodado -->
                `;
